...
contract WaBUSDToken is AccessControl, ERC20("Wasabi BUSD", "waBUSD") {
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "waBUSD: Alchemist is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "waBUSD: Alchemist is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"waBUSD: Alchemist's ceiling was breached.");
    require(!paused[msg.sender], "waBUSD: user is currently paused.");
