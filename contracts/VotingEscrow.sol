...
contract VotingEscrow is IERC20 {
    ...
    address[] public rewardVestings;
    bool[] public needVestings;
    ...
    function initialize(IMintableERC20 _wasabi,
        ...
        rewardVestings = _rewardVestings;
        collector = _collector;
        needVestings = _needVestings;
        initialized = true;
    ...
    function addRewardToken(address _newRewardToken, address _newRewardVesting, bool _needVesting) external onlyGovernance expectInitialized {
        require(_newRewardToken != address(0), "VotingEscrow: new reward token address cannot be 0x0");

        if (_needVesting) {
            require(_newRewardVesting != address(0), "VotingEscrow: new reward vesting address cannot be 0x0");
        }

        rewardTokens.push(_newRewardToken);
        rewardVestings.push(_newRewardVesting);
        needVestings.push(_needVesting);

        emit RewardTokenAdded(_newRewardToken, _newRewardVesting, _needVesting);
    }
        ...
        if (veBal > 0) {
            ...
            for (uint i=0; i<rewardTokens.length; i++) {
                ...
                if (tokenAddress != address(0)) {
                    ...
                    if (pending > 0) {
                        bool needVesting = needVestings[i];
                        if (needVesting) {
                            address rewardVestingAddress = rewardVestings[i];
