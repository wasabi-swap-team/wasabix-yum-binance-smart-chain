...
let YumVaultFactory: ContractFactory;
...
let TransmuterAdapterMockFactory: ContractFactory;

describe("MigratableBUSDTransmuterV2", () => {
  let deployer: Signer;
  ...
  let transmuter: MigratableBusdTransmuterV2;
  ...
  before(async () => {
    TransmuterV2Factory = await ethers.getContractFactory("MigratableBUSDTransmuterV2");
    ERC20MockFactory = await ethers.getContractFactory("ERC20Mock");
    AlUSDFactory = await ethers.getContractFactory("WaToken");
    YumVaultFactory = await ethers.getContractFactory("YumAlpacaBUSDVault");
    VaultAdapterMockFactory = await ethers.getContractFactory(
      "VaultAdapterV2Mock"
    );
    TransmuterAdapterMockFactory = await ethers.getContractFactory(
  ...
  beforeEach(async () => {
    ...
    alchemist = (await YumVaultFactory.connect(deployer).deploy(
      token.address,
      alUsd.address,
    ...
    transVaultAdaptor = (await TransmuterAdapterMockFactory.connect(deployer).deploy(
        token.address
      )) as TransmuterV2AdapterWithIndirectionMock;
    await transmuter.connect(governance).setActiveVault(transVaultAdaptor.address);
  ...
  describe("unstake()", () => {
    ...
    it("reverts on depositing and then unstaking balance greater than deposit", async () => {
      ...
      expect(transmuter.unstake(utils.parseEther("2000"))).revertedWith(
        "MigratableBUSDTransmuterV2: unstake amount exceeds deposited amount"
      );
    });
  ...
  describe("forceTransmute()", () => {
    ...
    it("reverts when you are not overfilled", async () => {
      ...
      expect(transmuter.connect(minter).forceTransmute(await depositor.getAddress())).revertedWith("MigratableBUSDTransmuterV2: !overflow");
    });
  ...
  describe("distribute()", () => {
    ...
    it("must be whitelisted to call distribute", async () => {
      ...
      ).revertedWith("MigratableBUSDTransmuterV2: !whitelisted")
    });
    ...
    describe("recallAllFundsFromVault()", async () => {
      ...
      it("reverts when not paused", async () => {
        expect(transmuter.connect(governance).recallAllFundsFromVault(0))
          .revertedWith("MigratableBUSDTransmuterV2: not paused, or not governance or sentinel")
      });
      ...
      it("recalls funds from any non-active vault", async () => {
        ...
        await transmuter.connect(governance).setActiveVault(newVault.address);
  ...
  describe("migrateFunds()", () => {
    ...
    let stakeAmt = parseEther("50");
    let distributeAmt = parseEther("100");
    let newTransmuter: MigratableBusdTransmuterV2;
    let newTransVaultAdaptor: TransmuterV2AdapterWithIndirectionMock;
    ...
    beforeEach(async () => {
      ...
      newTransVaultAdaptor = (await TransmuterAdapterMockFactory.connect(deployer).deploy(
        token.address
      )) as TransmuterV2AdapterWithIndirectionMock;
      newTransmuter.connect(governance).setActiveVault(newTransVaultAdaptor.address);
    ...
    it("reverts if anyone but governance tries to migrate", async () => {
      expect(transmuter.connect(depositor).migrateFunds(newTransmuter.address))
        .revertedWith("MigratableBUSDTransmuterV2: !governance");
    });
    ...
    it("reverts if not in emergency mode", async () => {
      expect(transmuter.connect(governance).migrateFunds(newTransmuter.address))
        .revertedWith("MigratableBUSDTransmuterV2: set pause first");
    });
